...
function getClarification(query: string, intent: ChatIntent): ChatResponsePayload | null {
  ...
  if (intent === "fees") {
    ...
    if (!className) {
      return {
        answer:
          "Aap kis class ki fee puchh rahe hain? Example: Class 10, Class 12 ya Competition.",
        source: "clarification",
        intent,
        needsClarification: true,
        clarificationKey: "class",
        suggestions: ["Class 10 fee", "Class 12 fee", "Competition fee"],
      };
    }
  }
  ...
  if (intent === "results") {
    ...
    if (!asksWhen && asksType) {
      return {
        answer:
          "Aapko result link chahiye, TSE result dekhna hai, ya certificate download karna hai?",
        source: "clarification",
        intent,
        needsClarification: true,
...
export async function getChatResponse(query: string): Promise<ChatResponsePayload> {
  ...
  const cacheKey = `chat:${normalized}`;
  ...
  if (documents.length > 0) {
    const aiAnswer = await generateStrictAnswer(query, documents);
  ...
  const fallback: ChatResponsePayload = {
    answer: "Please contact institute",
    source: "fallback",
    intent: rule.intent,
    suggestions,
  };
